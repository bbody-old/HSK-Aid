...
public class Excel {
	...
	public Excel(String filename) throws InvalidFormatException, IOException {
		...
		for (Iterator<Row> row = sheet.rowIterator(); row.hasNext();) {
			...
			for (Iterator<Cell> cell = row.next().cellIterator(); cell
					.hasNext();) {
				columnsOfRow++;
				Cell c = cell.next();
				try {
	...
	public String[][] getValues() {
		String[][] values = new String[Math.min(rowsSize, 5)][Math.min(
				columnsSize, 5)];
	...
	public Cards getCards(int titleRow, int englishColumn, int chineseColumn,
			int pinyinColumn) {
		...
		for (int i = titleRow + 1; i < excel.size(); i++) {
			...
			if (temp.size() > 3) {
				Card c = new Card(temp.get(chineseColumn),
						...
						temp.get(englishColumn), temp.get(pinyinColumn));
